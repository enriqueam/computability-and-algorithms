...
// Modificacion. Genero un lenguaje con las subcadenas del propio lenguaje.
Lenguaje Lenguaje::ModiSubcadenas() {
  Lenguaje Lenguaje_modificado;
  Lenguaje_modificado.alfabeto_ = alfabeto_;
  for (auto cadena : lenguaje_) {
    std::vector<Cadena> subcadenas = cadena.Subcadenas();
    for (auto subcadena : subcadenas) {
      Lenguaje_modificado.InsertarCadena(subcadena);
    }
  }
  return Lenguaje_modificado;
}