...
// Modificacion
void AnalisisFichero::CazarModificacion() {
  std::regex expresion("(\\d+|\\w+)\\s(\\+|\\*|\\/|%|-)\\s(\\d+|\\w+)");
  std::regex operando1("(\\d+|\\w+)\\s");
  std::regex operando2("\\s(\\d+|\\w+)");
  std::smatch expresion_match;
  std::smatch expresion_match1;
  std::smatch expresion_match2;
  for (unsigned int index = 0; index < entrada_.size(); index++) {
    if (std::regex_search(entrada_[index], expresion_match, expresion)) {
      std::string expresion = expresion_match[0];
      DetectarOperacion DetectarOperacion(expresion_match[2], expresion_match[1], expresion_match[3], index + 1);
        expresiones_.push_back(DetectarOperacion);
    }
  }
}
...
// Modificacion
std::vector<DetectarOperacion> AnalisisFichero::GetExpresiones() {
  return expresiones_;
}