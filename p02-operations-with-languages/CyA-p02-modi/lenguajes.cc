...
Lenguaje Lenguaje::Potencia() {
  ...
  if (potencia == 0) {
    ...
    for (int index = 0; index < potencia; index++) {
      Lenguaje_potenciado = Concatenacion(*this);
    }
  }
...
Lenguaje Lenguaje::ModiSubcadenas() {
  Lenguaje Lenguaje_subcadenas;
  Lenguaje_subcadenas.alfabeto_ = alfabeto_;
  std::vector<Cadena> subcadenas;
  for (auto cadena : lenguaje_) {
    std::vector<Cadena> subcadenas = cadena.Subcadenas();
    for (auto subcadena : subcadenas) {
      Lenguaje_subcadenas.InsertarCadena(subcadena);
    }
  }
  return Lenguaje_subcadenas;
}