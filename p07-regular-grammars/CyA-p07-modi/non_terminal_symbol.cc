...
std::vector<NonTerminalSymbol>* NonTerminalSymbol::GetNextNonTerminalSymbols(Symbol& symbol) const {
  std::vector<NonTerminalSymbol>* next_non_terminal_symbols = new std::vector<NonTerminalSymbol>();
  for (Production production : *productions_) {
    if (production.GetSymbol() == symbol) {
      next_non_terminal_symbols->push_back(*production.GetNonTerminalSymbol());
    }
  }
  return next_non_terminal_symbols;
}

bool NonTerminalSymbol::GetIsFinal() const {
  return is_final_;
}
