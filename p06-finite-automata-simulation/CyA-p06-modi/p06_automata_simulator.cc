...
int SearchState(std::vector<State> vector_states, std::string id_state) {
  for (unsigned int index = 0; index < vector_states.size(); index++) {
    std::cout << vector_states[index].GetId() << std::endl;
    std::cout << id_state << std::endl;
...
void IdentifyDeterministic(FiniteAutomata& finiteAutomata, Alphabet& alphabet, std::vector<std::string> fa_input) {
  int count = 0;
  for (unsigned int index = 3; index < fa_input.size(); index++) {
    std::string line = fa_input[index];
    int number_transitions = std::stoi(std::string(1, line[4]));
    if (number_transitions != alphabet.Size()) {
      count++;
    }
  }
  
  if (count != alphabet.Size()) {
    std::cout << "El autómata es determinista." << std::endl;
  } else {
    std::cout << "El autómata no es determinista." << std::endl;
  }
}
...
int main(int argc, char *argv[]) {
  if (CheckInput(argc, argv)) {
    ...
    IdentifyDeterministic(FiniteAutomata, alphabet, fa_input);
